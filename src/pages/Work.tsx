...
const ProjectCard = memo<{ project: typeof PROJECTS[0]; index: number }>(({ project, index }) => {
  ...
  return (
    <article className="group relative flex-none w-[85vw] md:w-[600px] h-full bg-bg-sub border-r border-border-main last:border-r-0 flex flex-col snap-start">
      ...
      <div className="p-8 flex flex-col flex-grow justify-between">
        ...
        <div className="flex gap-6 pt-6 border-t border-border-main">
          {project.links.demo && (
            <a href={project.links.demo} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[10px] font-bold text-text-main hover:text-accent transition-colors uppercase tracking-widest font-mono">
              LIVE_DEMO <ArrowRight size={12} className="-rotate-45" /> 
            </a>
          )}
          {project.links.repo && (
            <a href={project.links.repo} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[10px] font-bold text-text-muted hover:text-text-main transition-colors uppercase tracking-widest font-mono">
      ...
      */}
      <div className="w-full overflow-x-auto pb-8 hide-scrollbar cursor-grab active:cursor-grabbing">
        <div className="flex w-max border-y border-border-main">
          {/* Spacer to align first item with container padding */}
